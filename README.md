...
- **Original music & sound effects, synthesized live with the Web Audio API** (no audio files): a gentle music-box loop in C pentatonic that automatically ducks while Pip speaks, a soft "boop" on every tap, a rising arpeggio + star burst on correct answers, a warm two-note "hmm" on misses, dings as stars land, and a fanfare at level complete. One button mutes voice, music, and effects together.
