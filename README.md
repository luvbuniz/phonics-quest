...
- **Original music & sound effects, synthesized live with the Web Audio API** (no audio files): a gentle music-box loop in C pentatonic that automatically ducks while Pip speaks, a soft "boop" on every tap, a rising arpeggio + star burst on correct answers, a warm two-note "hmm" on misses, dings as stars land, and a fanfare at level complete. Music and sounds/voice have separate toggles, so either can be switched off independently.
- **Collectible badges (IXL-style):** every level awards a unique hand-drawn badge medallion on first completion, and mastering the skill (90+) turns the badge gold. A kid-facing badge case on the map screen shows the collection, with unearned badges as mystery silhouettes.
